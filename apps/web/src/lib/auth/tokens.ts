...
export async function signAccessToken(claims: Omit<AccessTokenClaims, "iat" | "exp" | "iss" | "aud">) {
  const env = getEnv();
  // RS256 requires a KeyObject (PEM bytes/Uint8Array are only valid for HS256)
  const key = createPrivateKey(env.JWT_PRIVATE_KEY.replace(/\\n/g, "\n"));
...
/** Verify an access token. Throws on invalid/expired. Public key only. */
export async function verifyAccessToken(token: string): Promise<AccessTokenClaims> {
  const env = getEnv();
  const key = createPublicKey(env.JWT_PUBLIC_KEY.replace(/\\n/g, "\n"));
